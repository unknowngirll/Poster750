...
def parse_bracken(infile, krakenfile):
    ...
    for file in file_list:
        ...
        sample_raw = file_lookup[file]

    # Collapse replicates into WHA, WHB, KHA, KHB
        if sample_raw.startswith("WHA"):
            sample = "WHA"
        elif sample_raw.startswith("WHB"):
            sample = "WHB"
        elif sample_raw.startswith("KHA"):
            sample = "KHA"
        elif sample_raw.startswith("KHB"):
            sample = "KHB"
        else:
            sample = sample_raw  # fallback
        ...
        with open(file, 'r') as f:
            ...
            for l in f:
                ...
                X = l.rstrip().split('\t')
                classification = X[0]
                if classification == "Homo sapiens":
        ...
        total_after_filtering = sum(smpldata.values())
        if total_after_filtering > 0:
            for clss in smpldata:
                smpldata[clss] = smpldata[clss] / total_after_filtering
                
        if sample not in data:
            data[sample] = []
        data[sample].append(smpldata)

    # reorganize raw data to prep for data frame
    data2 = defaultdict(list)

    for smpl, replicate_list in data.items():
        data2['Sample'].append(smpl)
        
        for clss in Classifications.keys():
            values = []
            for replicate in replicate_list:
                values.append(replicate.get(clss, 0))
            
            # average replicates (change to sum(values) if desired)
            data2[clss].append(sum(values) / len(values))
